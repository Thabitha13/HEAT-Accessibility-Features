...
public class ConsoleWindow {
  ...
  private void jbInit() throws Exception {
    ...
    /*
     * Use a CaretListener to ensure that the caret can only appear after the fixed
     * content.
     * Use a listener instead of a filter, so that still any text (including fixed)
     * can be marked for
     * copying.
     */
  ...
  /*
   * Note that historyIterator always points too backwards in list, have to skip
   * one element.
   * That is because a ListIterator points between elements, not at an element as
   * needed here.
   */
  ...
  /**
   * Get command string currently in console command; i.e. all text after last
   * prompt.
   * 
  ...
  /**
   * Add character to end of console output, doing syntax-highlighting of possible
   * error message
   * 
  ...
  public void setEnabled(boolean b) {
    // do not disable GUI component, because that would make console output close to
    // invisible,
 ...
 public void refreshStyles() {
  System.out.println("refreshStyles START");
    ...
    StyleConstants.setForeground(promptText, ColorThemeManager.getPromptColor());
     ...
     javax.swing.text.StyledDocument styledDoc =
         (javax.swing.text.StyledDocument) jtaIODoc;
    
     int len = styledDoc.getLength();
     for (int i = 0; i < len; i++) {
         javax.swing.text.Element el = styledDoc.getCharacterElement(i);
         Color c = StyleConstants.getForeground(el.getAttributes());
         if (c.equals(ColorThemeManager.DeuteranopiaTheme.ERROR_COLOR)
                 || c.equals(ColorThemeManager.DefaultTheme.ERROR_COLOR)) {
             styledDoc.setCharacterAttributes(i, 1, errorText, false);
         } else if (c.equals(ColorThemeManager.DeuteranopiaTheme.PROMPT_COLOR)
                 || c.equals(ColorThemeManager.DefaultTheme.PROMPT_COLOR)) {
             styledDoc.setCharacterAttributes(i, 1, promptText, false);
         }
     }
     jtaInterpreterOutput.repaint();
